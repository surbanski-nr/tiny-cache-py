    ...
    try:
        async with CacheClient("localhost:50051", timeout=5.0) as client:
            ...
            if await client.ping():
                ...
                print("Cache service is not responding")
                return

            # Test set operation
            print("\n--- Testing SET operations ---")
            success = await client.set("test_key", "hello world", ttl=60)
            print(f"Set 'test_key': {'OK' if success else 'FAIL'}")
            
            success = await client.set("number_key", 42)
            print(f"Set 'number_key' (int): {'OK' if success else 'FAIL'}")
            
            success = await client.set("bool_key", True)
            print(f"Set 'bool_key' (bool): {'OK' if success else 'FAIL'}")
            ...
            print(f"Delete 'test_key': {'OK' if deleted else 'FAIL'}")

            value = await client.get("test_key")
            print(f"Get deleted key: {value}")

            deleted = await client.delete("non_existent")
            print(f"Delete non-existent key: {'OK' if deleted else 'FAIL'}")
